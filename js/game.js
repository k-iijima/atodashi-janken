...
import { TIMING, FRAMES, MOTION, COUNT_FROM } from "./config.js";
...
let stillCount = 0;   // 手首が連続で静止しているフレーム数(振りとの区別用)
 ...
 * @param {number} motion - 手首の1フレームあたり移動量(見えなければ Infinity)
 */
export function onFrame(raw, present, motion) {
  // --- 手の在・不在と静止を数える ---
  if (present) {
    presentCount++;
    missCount = 0;
    // 静止:振り(じゃん・けん)の折り返し点でも1〜2フレームは止まって
    // 見えるため、連続カウントで「本当に止めた手」だけを拾う
    stillCount = motion < MOTION.STILL ? stillCount + 1 : 0;
  } else {
    presentCount = 0;
    missCount++;
    stillCount = 0;
  ...
  // --- 判定中:手が安定したら当機が後出しする ---
  // 「ぽん!の瞬間から出しっぱなしの手」(振りの残りのグー等)は、本当に
  // その手を出すつもりなのか、これから変わる途中なのか分からないので
  // 長め(CARRY フレーム)に粘る。ただし手首が静止していれば振りの途中では
  // ない(振りの拳は動き続ける)ので、粘りを免除して即確定する。
  // これがないと「グーを普通に出した人」だけ毎回判定が0.4秒遅くなる。
  if (state === "judge" && candidate !== null) {
    const carried = candidate === swingHand;
    const settled = stillCount >= FRAMES.STILL;
    const need = carried && !settled ? FRAMES.CARRY : FRAMES.STABLE;
