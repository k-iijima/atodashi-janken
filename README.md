...
当機が出した直後に手を変えると「後出し!」として**貴殿の反則負け**(証拠の**犯行写真**つき)。手を引っ込めるだけならセーフ。場内掲示板には対戦数・勝率(常に100%)・後出し時間(ms)・貴殿の反則回数が表示される。
...
反則検出も同じトレードオフの裏返しで、勝負判定より厳しめの5フレーム一致にしないと、手を引っ込める途中の誤認識で無実の人が反則負けになる。それでも勝負後に手を下ろすだけで冤罪が多発したため、最終的に「反則監視は当機が出した直後の0.8秒だけ」+「判定の瞬間を写真で凍結して表示」に落ち着いた。本気で出し直す人は当機の手を見てから変えるので、この窓に必ず収まる。
