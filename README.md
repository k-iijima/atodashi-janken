...
出すのが少し遅れて判定とズレても、直後(0.5秒以内)なら当機が「**判定写真(改)**」として判定をやり直してくれる(それでも勝つ)。それより遅い手替えは当機の手を見てから変えたとみなし、「後出し!」として**貴殿の反則負け**(証拠の**犯行写真**つき)。手を引っ込めるだけならセーフ。場内掲示板には対戦数・勝率(常に100%)・後出し時間(ms)・貴殿の反則回数が表示される。
...
一番しぶとかったのは**境界ギリギリ問題**:「じゃん、けん」で振っている拳(グー)が「ぽん!」直後に先に確定してしまい、本当の手が一瞬遅れて届くと毎回「手替え=後出し」にされる。対策は二段構えで、(1) ぽん!の瞬間から出しっぱなしの手は確定を約0.4秒粘る(振りの残りを即決しない)、(2) それでも確定直後0.5秒以内に届いた別の手は反則ではなく「判定写真(改)」として判定をやり直す。人間が当機の手を見てから変えるには最短0.4〜0.6秒かかるので、それより速い手替えは「元々出すつもりだった手」と推定できる、というのが猶予の根拠。
